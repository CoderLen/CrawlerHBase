		...
		public void map(ImmutableBytesWritable key,Result values,Context context) throws IOException, InterruptedException{
			...
			for(Cell cell : values.listCells()){
				if("doc".equals(Bytes.toString(CellUtil.cloneFamily(cell))) && "document".equals(Bytes.toString(CellUtil.cloneQualifier(cell)))){
					...
					put.add(Bytes.toBytes("url"), Bytes.toBytes("redirect"), Bytes.toBytes(redirect.toString()));
					put.add(Bytes.toBytes("url"), Bytes.toBytes("timeStamp"), Bytes.toBytes(format.format(new Date())));
					put.add(Bytes.toBytes("url"), Bytes.toBytes("outLinks"), Bytes.toBytes(outlinks));
					put.add(Bytes.toBytes("url"), Bytes.toBytes("typeOfOutlink"), Bytes.toBytes(typeOfOutlink));
